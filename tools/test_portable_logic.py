...
with tempfile.TemporaryDirectory(prefix="xspark-logic-") as tmp:
    ...
    for file in ['Strategy/ScoringEngine.mqh', 'Core/StateStore.mqh', 'Risk/RiskManager.mqh', 'Strategy/ScoreBotV3.mqh',
                 'Strategy/CandleFlow.mqh', 'Strategy/TrendScalp.mqh', 'Strategy/ICTLiquidity.mqh',
                 'Strategy/SmartMoney.mqh']:
    ...
    body += adapt((ROOT / 'MQL5/Scripts/Tests/TestICTLiquidity.mq5').read_text())
    body += adapt((ROOT / 'MQL5/Scripts/Tests/TestSmartMoney.mq5').read_text())
    ...
    for ea in ['MQL5/Experts/XSpark/XSpark.mq5', 'MQL5/Experts/XSparkFlow/XSparkFlow.mq5',
               ...
               'MQL5/Experts/XSparkSMC/XSparkSMC.mq5']:
    ...
    body += position_tests
    source.write_text(body + '\nint main() { OnStart(); TestBoundaries(); RunChartPatternTests(); RunEntrySettingsTests(); MultiPositionTests::Run(); RunConcurrentRiskTests(); RunCandleFlowTests(); RunStrategyIdentityTests(); RunTrailingStopTests(); RunProfitLadderTests(); RunTrendScalpTests(); RunIctTests(); RunSmcTests(); Print("TOTAL passed=",g_passed+g_pattern_passed+g_settings_passed+g_concurrent_passed+g_flow_passed+g_identity_passed+g_trail_passed+g_ladder_passed+g_scalp_passed+g_ict_passed+g_smc_passed," failed=",g_failed+g_pattern_failed+g_settings_failed+g_concurrent_failed+g_flow_failed+g_identity_failed+g_trail_failed+g_ladder_failed+g_scalp_failed+g_ict_failed+g_smc_failed); return g_failed+g_pattern_failed+g_settings_failed+g_concurrent_failed+g_flow_failed+g_identity_failed+g_trail_failed+g_ladder_failed+g_scalp_failed+g_ict_failed+g_smc_failed ? 1 : 0; }\n')
    exe = Path(tmp) / 'logic'
