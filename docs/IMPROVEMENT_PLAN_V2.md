...
| (e) | The EA does not perform when `InpMaxOpenTrades` is set above 1. |

(a) through (d) are mechanically explainable from source, and are largely the same defect seen from four sides. **(e) is a separate cluster with six distinct causes** (section 2.8), four of which are live defects rather than tuning. This plan commits to:
...
### 2.8 (e) The concurrency path has six defects, and two of them are safety-relevant

`InpMaxOpenTrades` defaults to 1 (`XSpark.mq5:52`) and `XSpark.mq5:261-263` refuses any value other than 1 or 2. **"More than 1" can therefore only ever mean 2**; a value of 3 or more is a CRITICAL refusal at `OnInit`, not a silent clamp.

**1. The account risk cap saturates exactly at two positions.** All three tiers default to 3.0 % (`XSpark.mq5:84-86`) and `InpMaxAccountRiskPct` to 6.0 (`:94`). `XSparkAccountRiskWithinCap` (`RiskManager.mqh:152, 161`) computes `projected_pct = (open_risk_cash + prospective_risk_cash) / balance * 100` and refuses on `projected_pct > max_account_risk_pct`. Two positions at 3.0 % is **6.00 % against a 6.0 % cap** — it passes on the strict inequality and by nothing else.

| Position 1 realised risk | Projected | Second entry |
| --- | --- | --- |
| exactly as sized, 3.00 % | 6.00 % | passes |
| 110 % of sized, 3.30 % | 6.30 % | **refused** |
| 125 % of sized, 3.75 % | 6.75 % | **refused** |

And `open_risk_cash` is computed from the **live** `POSITION_SL` and `POSITION_VOLUME` (`XSpark.mq5:628-634`), i.e. from realised risk, not sized risk. Section 2.4 and ADR-015 both record that realised risk routinely exceeds sized risk — entry slippage inside `InpEntryDeviationPoints` plus stop-level adjustment that moves the stop outward only — and the gold defaults print a startup WARNING saying a fill can realise **125 %** of selected risk. **So the second position is refused on a large fraction of bars, and the operator sees an `ACCOUNT RISK` rejection rather than a second trade.** Setting 2 does not reliably give 2; it gives an unpredictable mixture of 1 and 2.

**2. The open-risk scan counts every position on the account.** `XSparkOpenAccountRiskCash` (`XSpark.mq5:609-641`) iterates `PositionsTotal()` with **no symbol filter and no Magic Number filter**. That is deliberate and the input comment at `:89-93` says so — it is how AGENTS.md rule 27 is enforced across instances. But the consequence at `InpMaxOpenTrades = 2` is worth stating: a manual trade on any symbol, or another EA's position, consumes XSpark's 6 % budget and refuses XSpark's second entry, with a rejection reason that names risk rather than the foreign position.

**3. Nothing prevents the two positions being opposite.** No check of any existing position's direction appears in `ExecutionEngine.mqh`, `RiskManager.mqh` or `SafetyManager.mqh`. ADR-017 declares hedging accounts, so a BUY and a SELL can be open on the same symbol simultaneously. Direction comes from candle geometry alone (section 2.5), so a bullish pin on one bar and a bearish pin four bars later produce a **locked spread**: both legs pay spread and swap, one is guaranteed to lose, and the pair cannot be resolved by either leg's take-profit without leaving the other naked. **At `InpMaxOpenTrades = 1` this state is unreachable. At 2 it is reachable on any bar where the pattern flips**, which under today's context-free detectors is common.

**4. Sizing ignores existing exposure, so two positions are one bet at double size.** `XSparkRiskPercentForScore` returns the tier percentage and `PositionSizer` applies it to `ACCOUNT_BALANCE`, with no term for open exposure. Two positions on the same symbol, same direction, from the same signal generator, in the same regime are not two bets — they are one bet at 2x. **ADR-022's growth-optimal 3.58 % is a per-bet fraction.** At two correlated concurrent positions the effective fraction is 6-7 %, and ADR-022's own arithmetic puts the zero-growth crossing near 7.2 %: above it a genuinely positive edge still shrinks the account, because compounding is multiplicative. **This is the most likely reason performance degrades rather than improves**, and it does so without any error, rejection or log line.

**5. The startup drawdown tolerance is wrong by a factor of two when 2 is set.** `XSpark.mq5:369-370` calls `XSparkConsecutiveLossesToDrawdown(InpMaxRiskPct, ...)`, whose model is `1 - (1-f)^k >= limit` for a **sequence** of independent single-trade losses (`RiskManager.mqh:47-56`). It returns 9 at 3.5 %/25 %. With two correlated concurrent positions one adverse move closes both, so the number of adverse *moves* to the killswitch is roughly **5, not 9** — and the startup line reports 9 regardless of `InpMaxOpenTrades`. The `XSPARK_MIN_LOSS_STREAK_TOLERANCE` warning at `:381` is computed from the same wrong figure and therefore stays silent exactly when it should fire.

**6. The two entries are maximally correlated by construction.** Signals are evaluated once per closed bar and `XSparkSignalBarIsSubmittable` (`ExecutionMath.mqh:58-62`) blocks a second submission on the same signal bar. So the two positions necessarily come from **different bars of the same move** — sequential entries into one trend leg, which is the worst possible correlation structure for a concurrency feature whose only justification would be diversification.

> **The honest summary of (e).** `InpMaxOpenTrades = 2` doubles the money at risk on a single correlated bet, past the growth optimum, while the cap that was supposed to bound it saturates exactly and fires inconsistently, the hedged-pair state becomes reachable, and the ruin-stop tolerance reported at startup is overstated twofold. **`docs/IMPROVEMENT_PLAN.md` section 7 already refused `InpMaxOpenTrades = 2`** on the correlation argument alone — "doubles cost-paying events per unit time while the bets are not independent: same symbol, same regime, overlapping." That refusal was correct, and the operator's observation is the empirical confirmation of it.
...
### S2b — Concurrency correctness. **Independent of every other stage. Ships early.**

Addresses finding (e). It depends on nothing in S3-S9 and nothing in S3-S9 depends on it, so it is ordered here purely because two of its parts are safety-relevant and one is a reporting lie.

| Field | Value |
| --- | --- |
| **Changes** | **(a)** Opposite-direction refusal: block a new entry whose direction opposes any live XSpark position on this symbol and Magic Number, with block reason `OPPOSING EXPOSURE`. Not an input — a hedged pair from one context-free signal generator is never intended behaviour. **(b)** `XSparkConsecutiveLossesToDrawdown` gains a `concurrent_positions` argument and the startup line at `XSpark.mq5:369-379` reports the tolerance for the configured `InpMaxOpenTrades`, so the `XSPARK_MIN_LOSS_STREAK_TOLERANCE` warning fires on the real number. **(c)** `OnInit` refuses `InpMaxOpenTrades = 2` while `InpMaxOpenTrades * max(tier risk) > InpMaxAccountRiskPct * 0.9` — the cap must have headroom for realised-over-sized risk, or the second entry is decided by slippage rather than by policy. **(d)** The `ACCOUNT RISK` rejection reason names the foreign-position contribution separately from XSpark's own, so a refusal caused by a manual trade on another symbol is legible as such. |
| **Files** | `SafetyManager.mqh`, `RiskManager.mqh`, `ExecutionEngine.mqh`, `XSpark.mq5`, `TestScoreBotV3Logic.mq5` |
| **Expectations** | At `InpMaxOpenTrades = 1`: **trade set byte-identical** to the control run — every part of this stage is unreachable at 1. At 2: the hedged-pair population drops to exactly 0 (a correctness check, not a result); the reported loss tolerance halves; the `OnInit` refusal fires on the shipped 3.0/6.0 defaults, which **is the intended outcome** and is the mechanical statement that those two numbers are inconsistent with each other. |
| **Revert if** | The `InpMaxOpenTrades = 1` trade set differs at all. |
| **Not in this stage** | Sizing that accounts for open correlated exposure. That is the real fix for cause 4 and it is a change to the money axis, so it belongs to a risk stage with its own evidence, not to a correctness stage. Until it exists, **`InpMaxOpenTrades = 1` is the only configuration this plan supports**, and (c) makes that mechanical rather than advisory. |
...
5. **Concurrency is refused rather than repaired, and the operator may not want that answer.** S2b makes `InpMaxOpenTrades = 2` refuse to start on the shipped defaults instead of making it work. That is deliberate: the cap saturates exactly, the bets are maximally correlated, and correlated concurrency past the growth optimum is the one failure mode where a positive edge still shrinks the account. **Making 2 genuinely safe needs exposure-aware sizing** — the second position sized against remaining risk budget rather than full balance — **plus a reason for the two entries to be independent bets**, which a single signal generator on a single symbol and a single regime cannot supply. Neither is in this plan. If concurrency matters to the account owner, the honest route is a second uncorrelated instrument, not a second slot on the same one.
