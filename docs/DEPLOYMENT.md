...
- Expect a WARNING on this line at the shipped gold defaults. 30 points against a 120-point minimum stop is a 25% realised-risk overshoot, so a permitted fill can realise up to 125% of selected risk. That is accurate, not a fault, and the threshold is deliberately not set to hide it. Lowering `InpEntryDeviationPoints` tightens realised risk at the cost of more rejected entries.
