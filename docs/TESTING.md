...
MQL5/Scripts/Tests/TestSmartMoney.mq5
```
...
`TestTrailingStop.mq5` and `TestProfitLadder.mq5` additionally prove that every style an operator can select resolves to a configuration its own validator accepts, and that an unknown style is refused rather than guessed. With the individual numbers gone from the Inputs tab those tables *are* the reachable set, so this is rule 47 applied to all of it rather than only to the shipped default.

`python3 tools/check_ea_inputs.py` enforces per-strategy input isolation across every Expert Advisor: that no input identifier is declared by two EAs, that every declared input is actually read by its own EA, that each EA's Magic Number default comes from the `StrategyIdentity` registry, and that no EA takes a default from another strategy's constants. It also checks the enums an EA uses as input types, because MetaTrader renders each member's trailing comment as the dropdown choice an operator reads: an unlabelled member shows the raw identifier, which is the failure rule 46 exists to prevent one level down. Verified by removing a label and watching the build fail. It is source analysis, not a compiler, and it runs in CI ahead of the logic suites. `MQL5/Scripts/Tests/TestStrategyIdentity.mq5` covers the registry itself: that the shipped Magic Numbers differ, that each strategy accepts its own and refuses the other's by name, that zero is refused, and that an operator's own unclaimed number stays usable on any strategy.
...
`MQL5/Scripts/Tests/TestSmartMoney.mq5` covers the Smart Money Concepts model, which is unusual in this repository in that it is a transcription: the checks exist to prove the published indicator was read correctly, not that the rule built on it is any good. So they pin the *source's* behaviour where it is surprising - the one-sided leg detector that consults nothing to the left of a pivot, the volatility parse that swaps a violent candle's high and low to deflate it out of the order block contest, the oldest-bar tie rule, and the rule that an internal break sitting at exactly the swing pivot's level is not internal structure at all - alongside the three places this implementation deliberately departs from it. The replay runs against a 160-candle fixture whose structure is known by construction, and is asserted twice on the same window to hold the determinism the whole design rests on. The rule on top is then exercised through hand-built states, one refusal per test, so the reason a fixture fails is never in doubt: every funnel stage, both directions, and every value of all four dropdowns. Two of those dropdown checks are rule 47 applied to a named choice rather than to a default - that each swing size fits the cached structure window, and that it leaves more replay behind it than one swing needs, because a dropdown value that can never produce a trade is a broken value rather than a conservative one.

Two behaviours the fixtures surfaced are worth knowing rather than rediscovering. An internal break at exactly the swing pivot's level is suppressed entirely, which is faithful and fires more often on synthetic paths than on real ones. And with the premium/discount filter on, the `NO DRAW` refusal is unreachable, because an entry in discount is by definition below the top of the dealing range - so that test runs with the zones off and says so, rather than passing vacuously.

`MQL5/Scripts/Tests/TestProfitLadder.mq5` covers the take-profit ladder's arithmetic: every configuration `XSparkValidateProfitLadder` refuses (a half-set level, a hole in the levels, steps too close together or running backwards, a total share above the ceiling, a share over 100%, an absurd distance, a hard target inside the last step), that the shipped `XSPARK_LADDER_DEFAULT_*` constants are themselves a valid ladder, the trigger prices in both directions, the cumulative share each step implies and the volume budget it leaves open, that even the largest legal ladder leaves a residual, that the downward scan takes the furthest step a price has reached rather than the nearest, and that a progress value which cannot be trusted makes the ladder inert rather than replaying it.

The `ManagePositions` fixtures then exercise the same logic through the real manager source against broker doubles: that a step closes a share of the *opening* volume rather than of the remainder, that it never fires twice, that it survives a restart, that a confirmed close which was never recorded is not replayed, that a gap through two steps banks both in one order, that a step with no legal volume is skipped and a later step makes good what it could not take, that a rejected close backs off and latches off after repeated rejections while the trail keeps running, that the ladder never runs in ScoreBot's trailing mode, that a position adopted without a persisted record is never laddered, and that neither the ladder nor the trail erases a live broker take-profit.

Two things these cannot cover. The broker's actual partial-close semantics - whether a given server accepts the volume, and what it does to the position id - are a demo-account question. And the result depends on the Strategy Tester's modelling mode: the ladder triggers on a tick-resolution quote while the trail's peak advances only on closed candles, so low-resolution modelling understates it.

`python3 tools/test_portable_logic.py` also compares, as source rather than behaviour, the three key lists that write, restore and delete per-position state. The portable storage doubles copy whole structs, so a key missing from one list is invisible to any behavioural fixture.
